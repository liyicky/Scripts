...
TESTFLIGHT_API_TOKEN = "!testflighttoken!"
TESTFLIGHT_TEAM_TOKENS = { :staging => "!stagingtoken!",
                           :production => "!productiontoken!" }
TESTFLIGHT_DISTRIBUTION_LISTS = { :staging => "!stagingdistrib!", :production => "!productiondistrib!" }
TRACKER_ID = "!trackerprojectid!"
...
task :build_for_device do
  ...
  system_or_exit(%Q[xcodebuild -project #{PROJECT_NAME}.xcodeproj -target #{PROJECT_NAME} -configuration #{build_configuration} -sdk iphoneos ARCHS=armv7 build SYMROOT=#{BUILD_DIR}], output_file("build_for_device"))
